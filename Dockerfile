...
# Where the `ws` / `dh` / `sc` shell helpers jump to.
ENV LIVE_ROOT=/app

WORKDIR /app

# Dependencies first, so editing a source file does not reinstall them.
COPY diamond-hands/package.json diamond-hands/package-lock.json ./diamond-hands/
RUN npm --prefix diamond-hands ci
...
# Surfpool's RPC. Publish it (`docker run -p 8899:8899`) to reach a validator
# running inside the container from the host.
EXPOSE 8899

WORKDIR /app/diamond-hands
CMD ["anchor", "test"]
